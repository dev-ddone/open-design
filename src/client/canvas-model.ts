...
export function ensureCanvasObjectIds(canvas: fabric.Canvas): void {
  for (const object of canvas.getObjects()) ensureObjectId(object);
}
...
export function normalizeEditRules(value: TemplateEditRules | null | undefined): TemplateEditRules {
  return {
    mode: value?.mode ?? "unlocked",
    editableObjectIds: value?.editableObjectIds ?? [],
    lockedObjectIds: value?.lockedObjectIds ?? [],
  };
}

export function isObjectEditable(
  object: fabric.FabricObject,
  rulesValue: TemplateEditRules | null | undefined,
): boolean {
