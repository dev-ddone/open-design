...
export function CanvasArea() {
  ...
  useEffect(() => {
    setZoomRaw(0.58);
  }, [canvasWidth, canvasHeight, setZoomRaw]);
  ...
  useEffect(() => {
    ...
    const handler = (event: WheelEvent) => {
      ...
      const nextZoom = Math.min(
        Math.max(previousZoom * (event.deltaY > 0 ? 0.95 : 1.05), 0.05),
        3,
      );
  ...
  const finishRename = () => {
    if (!readOnly && renamingId && renameValue.trim()) {
      void renamePage(renamingId, renameValue.trim());
    }
    setRenamingId(null);
  };
        ...
        >
                ...
                >
                  <div class="flex items-center gap-1.5">
                    {renamingId === page.id && !readOnly ? (
                      ...
                      <span
                        class={`text-[11px] text-zinc-400 font-medium transition-colors ${
                          readOnly ? "cursor-default" : "cursor-pointer hover:text-zinc-600"
                        }`}
                        onClick={() => {
                          if (!readOnly) {
                  ...
                  {!readOnly && (
                    <div class="flex items-center gap-0.5">
                      <button
                        class="p-1 rounded bg-transparent border-none cursor-pointer text-zinc-400 hover:text-[#6366f1] hover:bg-[#6366f1]/10"
                      ...
                      {pages.length > 1 && (
                        <button
                          class="p-1 rounded bg-transparent border-none cursor-pointer text-zinc-400 hover:text-red-500 hover:bg-red-500/10"
          ...
          {!readOnly && (
            <div style={{ height: 40 * inverseScale }}>
              ...
              >
                <button
                  class="flex items-center gap-1.5 px-4 py-2 rounded-lg border-2 border-dashed border-zinc-300 bg-transparent cursor-pointer text-xs text-zinc-400 font-medium hover:border-[#6366f1] hover:text-[#6366f1] hover:bg-[#6366f1]/5"
