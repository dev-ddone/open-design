...
interface EditorCommand { id: string; title: string; description: string; shortcut?: string; keywords: string; readOnly?: boolean; run: () => void; }
...
export function CommandPalette() {
  ...
  const commands = useMemo<EditorCommand[]>(() => [
    ...
    { id: "plugins", title: "Plugin e integrazioni", description: "Installa manifest, configura comandi e connettori", keywords: "plugin sdk integrazioni estensioni manifest", run: () => window.dispatchEvent(new Event("ddone:open-plugin-manager")) },
  ...
  useEffect(() => { if (!open) return; setSelected(0); window.setTimeout(() => inputRef.current?.focus(), 0); }, [open, query]);
  if (!open) return null;
  return <div class="fixed inset-0 z-[150] flex items-start justify-center bg-black/35 px-4 pt-[10vh]" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}><div class="w-full max-w-2xl overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-2xl" role="dialog" aria-modal="true" aria-label="Comandi e scorciatoie"><div class="flex items-center gap-3 border-b border-zinc-200 px-4"><Search size={17} class="text-zinc-400" /><input ref={inputRef} value={query} onInput={(event) => setQuery((event.target as HTMLInputElement).value)} placeholder="Cerca comandi, strumenti o scorciatoie…" class="h-14 flex-1 border-0 bg-transparent text-sm outline-none" /><kbd class="rounded border border-zinc-200 bg-zinc-50 px-2 py-1 text-[8px] text-zinc-500">ESC</kbd><button onClick={() => setOpen(false)} class="grid h-8 w-8 place-items-center rounded-lg border-0 bg-transparent text-zinc-400 cursor-pointer"><X size={15} /></button></div><div class="max-h-[60vh] overflow-y-auto p-2">{filtered.map((command, index) => <button key={command.id} onMouseEnter={() => setSelected(index)} onClick={() => { command.run(); setOpen(false); }} class={`flex w-full items-center gap-3 rounded-xl border-0 p-3 text-left cursor-pointer ${selected === index ? "bg-violet-50" : "bg-white"}`}><span class={`grid h-9 w-9 shrink-0 place-items-center rounded-xl ${selected === index ? "bg-violet-600 text-white" : "bg-zinc-100 text-zinc-500"}`}>{command.shortcut ? <Keyboard size={15} /> : <Command size={15} />}</span><span class="min-w-0 flex-1"><strong class="block text-[10px] text-zinc-800">{command.title}</strong><span class="mt-0.5 block text-[8px] text-zinc-400">{command.description}</span></span>{command.shortcut && <kbd class="rounded border border-zinc-200 bg-white px-2 py-1 text-[8px] text-zinc-500">{command.shortcut}</kbd>}{selected === index && <CornerDownLeft size={12} class="text-violet-500" />}</button>)}{filtered.length === 0 && <p class="p-10 text-center text-[10px] text-zinc-400">Nessun comando trovato.</p>}</div><footer class="flex items-center justify-between border-t border-zinc-100 px-4 py-2 text-[7px] text-zinc-400"><span>↑↓ naviga · Invio esegue · ESC chiude</span><span>Ctrl/⌘ K</span></footer></div></div>;
}